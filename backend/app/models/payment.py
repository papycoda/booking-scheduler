...
class Payment(UUIDPrimaryKeyMixin, Base):
    ...
    initialization_error: Mapped[str | None] = mapped_column(Text)
    initialization_attempts: Mapped[int] = mapped_column(Integer, nullable=False, server_default="0")
    last_initialization_attempt_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True))
